...
returning_clause "RETURNING Clause"
  = f:( RETURNING ) o t:( returning_target )
  {
    return {
      'returning': t
    };
...
returning_target
  = f:( returning_node ) o r:( returning_target_loop )*
  { return flattenAll([ f, r ]); }

returning_target_loop
  = sym_comma n:( returning_node ) o
  { return n; }

returning_node
  = returning_node_star
  / returning_node_aliased
...
stmt_insert "INSERT Statement"
  = k:( insert_keyword ) o t:( insert_target ) ret:( returning_clause )?
  {
    return Object.assign({
      'type': 'statement',
      'variant': 'insert'
...
stmt_update "UPDATE Statement"
  = s:( update_start ) f:( update_fallback )?
    ...
    o:( stmt_core_order )? o l:( stmt_core_limit )? ret:( returning_clause )?
  {
    return Object.assign({
      'type': 'statement',
      'variant': s,
...
stmt_delete "DELETE Statement"
  = s:( delete_start ) t:( table_qualified ) o w:( stmt_core_where )?
    o:( stmt_core_order )? l:( stmt_core_limit )? ret:( returning_clause )?
  {
    return Object.assign({
      'type': 'statement',
      'variant': s,
